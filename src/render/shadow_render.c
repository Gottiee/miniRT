...
/*   Updated: 2022/11/22 19:17:42 by gmansuy          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	shadow_render(t_record *rec, t_point light)
{
	...
	shadow.light = 1;
	rec->light_level = 1;
	//path.orig est pas bon probablement
	...
	{
		t_vec3 calc = minus(light, path.orig);
		rec->light_level *= dot(unit_vector(light), unit_vector(rec->normal));
		rec->light_level /= (length(&calc) / 3);
	}
}
